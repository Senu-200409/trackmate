...
function RegisterDriverModal({ isOpen, onClose, onSuccess }) {
  ...
  const validateStep1 = () => {
    ...
    if (!registerForm.tud_user_name.trim()) {
      errors.tud_user_name = 'Driver name is required';
    } else if (registerForm.tud_user_name.trim().length < 3) {
      errors.tud_user_name = 'Driver name must be at least 3 characters';
    }
  ...
  const handleStep1Next = async (e) => {
    ...
    try {
      ...
      console.log('Driver Step 1 FULL API response:', JSON.stringify(data));

      if (response.ok && data.StatusCode === 200) {
        console.log('Driver Step 1 complete — UserID:', data.UserID);
        localStorage.setItem('registerDriverUserID', data.UserID);

        // Fetch DriverID from auto-created record
        try {
      ...
      console.error('Driver Step 1 error:', err);
  ...
  const handleStep2Submit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const storedUserID = localStorage.getItem('registerDriverUserID');
      const driverID = localStorage.getItem('registerDriverID');

      console.log('Driver Step 2 — DriverID from localStorage:', driverID, 'UserID:', storedUserID);
      ...
      console.log('Driver Step 2 — Sending to PutDriverDetails:', {
        DriverID: driverID,
        UserID: storedUserID,
      ...
      const response = await fetch('https://trackmate.dockyardsoftware.com/DriverDetails/PutDriverDetails', {
        method: 'POST',
        body: formData,
      });
      const data = await response.json();
      console.log('Driver Step 2 FULL API response:', JSON.stringify(data));

      if (data?.StatusCode === 200 || response?.ok) {
        setRegisterSuccess('Driver registered successfully!');
        
        localStorage.removeItem('registerDriverUserID');
      ...
      console.error('Driver Step 2 error:', err);
      setError('Failed to connect to server. Please try again.');
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-[#1E3A5F]">Register Driver</h2>
          <button
            onClick={handleClose}
        ...
        <div className="p-6">
          ...
          {currentStep === 1 && (
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-[#5C5C5C] mb-4">Enter driver's basic details</p>
              </div>
          ...
          {currentStep === 2 && (
            <div className="space-y-4">
              <div>
                <h3 className="text-lg font-semibold text-[#1E3A5F] mb-2">License Information</h3>
                <p className="text-sm text-[#5C5C5C] mb-4">Complete license details</p>
              </div>
              ...
              <form onSubmit={handleStep2Submit} className="space-y-4">
                ...
                <div>
                  ...
                  />
                  {registerErrors.licenseNo && (
                    <p className="text-sm text-red-600 mt-1">{registerErrors.licenseNo}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Light Vehicle (LV)">Light Vehicle (LV)</option>
                    <option value="Motorcycle (MC)">Motorcycle (MC)</option>
                  </select>
                  {registerErrors.licenseType && (
                    <p className="text-sm text-red-600 mt-1">{registerErrors.licenseType}</p>
                  )}
                </div>
